...
export default function AlienCard({ alien, onSelect }) {
  ...
  return <button className="alien-card" style={{'--alien':alien.color}} onClick={()=>onSelect(alien)}>
    ...
    <div className="alien-portrait"><div className="portrait-rings"/><CharacterArt item={alien}/><span className="portrait-scan"/><span className="era-tag">{alien.era}</span></div>
    ...
    {alien.stats?<div className="mini-stats">{Object.entries(alien.stats).map(([name,value])=><div key={name}><span>{name}</span><i><motion.b initial={{scaleX:0}} whileInView={{scaleX:1}} viewport={{once:true}} transition={{duration:.65}} style={{width:`${value}%`,transformOrigin:'left'}}/></i><em>{value}</em></div>)}</div>:<div className="file-link">Explore transformation <span>↗</span></div>}
  </button>
}
